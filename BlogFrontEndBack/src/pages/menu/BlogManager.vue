<script setup lang="ts">
import type { Line } from '@antv/g2plot'
import { ref, reactive, onMounted, nextTick } from 'vue'
import { EditOutlined, DeleteOutlined, StopOutlined, CheckCircleOutlined } from '@ant-design/icons-vue'
import CrudTableModal from './components/crud-table-modal.vue'
import { ArticleVO } from 'types/Entity'
import { useRouter } from 'vue-router'
import { message } from 'ant-design-vue'
import { getArticleList, toggleArticleComment, getArticleListByCategory } from '~@/api/Article/ArticleList'
import { getRecentArticleDaily } from '~@/api/ArticleDaily'
import { getCategoryList } from '~@/api/Category'
import { useRoute } from 'vue-router'
import { formatTimer } from '~@/utils/tools'
import { deleteMyArticle } from '~@/api/Article/DeleteArticle'
const router = useRouter()
const route = useRoute()
let chartLibraryLoader: Promise<typeof import('@antv/g2plot')> | null = null

// --- 1. 手动定义状态 ---
const loading = ref(false)
const articleList = ref<ArticleVO[]>([])

const categories = ref<any[]>([])
const selectedCategoryId = ref<number | undefined>(undefined)

const fetchCategories = async () => {
  try {
    const res: any = await getCategoryList()
    categories.value = res.data || []
    
    // Check if categoryName is in query
    const queryCategory = route.query.categoryName as string
    if (queryCategory) {
      const cat = categories.value.find(c => c.name === queryCategory)
      if (cat) {
        selectedCategoryId.value = cat.id
        loadData()
      }
    }
  } catch (error) {
    console.error('获取分类失败', error)
  }
}


const pagination = reactive({
  current: 1,
  pageSize: 8, 
  total: 0,
  showSizeChanger: false, 
  onChange: (page: number, pageSize: number) => {
    pagination.current = page
    pagination.pageSize = pageSize
    loadData() 
  }
})

function loadChartLibrary() {
  if (!chartLibraryLoader)
    chartLibraryLoader = import('@antv/g2plot')

  return chartLibraryLoader
}

// 2. 加载数据逻辑
const loadData = async () => {
  loading.value = true
  try {
    const params: any = {
      page: pagination.current,
      size: pagination.pageSize
    }
    
    let res: any
    if (selectedCategoryId.value) {
      params.categoryId = selectedCategoryId.value
      res = await getArticleListByCategory(params)
    } else {
      res = await getArticleList(params)
    }
    
    if (res.code === 200) {
      articleList.value = res.data.records || []
      pagination.total = res.data.total
    }
  } catch (error) {
    console.error(error)
  } finally {
    loading.value = false
  }
}

// 初始化加载
onMounted(() => {
  fetchCategories()
  loadData()
})

const handleCategoryChange = () => {
  pagination.current = 1
  loadData()
}

// --- 4. 业务操作逻辑 ---

// 删除
async function handleDelete(id: number | string) {
  try {
    console.log('删除文章ID:', id)
    await deleteMyArticle(id) // 你的删除API
    message.success('删除成功')

    // 删除成功后刷新
    await loadData()
  } catch (e) {
    console.log(e)
  }
}

// 打开编辑
function handleEdit(record: any) {
  router.push({ name: 'edit', params: { id: record.id } })
}


// 禁评/允评
async function toggleCommentStatus(item: any) {
  try {
    const res: any = await toggleArticleComment(item.id)
    if (res.code === 200) {
      // 翻转 isComment: 0禁止 <-> 1允许
      item.isComment = item.isComment === 1 ? 0 : 1
      message.success(item.isComment === 1 ? '已允许评论' : '已禁止评论')
    } else {
      message.error(res.message || '操作失败')
    }
  } catch (e) {
    message.error('操作失败')
  }
}

// 跳转详情
const gotoDetail = async (id: any) => {
  router.push({ name: 'ArticleDetail', params: { id: id } })
}

// 评论管理相关
function handleCommentManage(record: any) {
  router.push({ name: 'commentmanager', params: { articleId: record.id } })
}

// === 数据可视化逻辑 ===
const dataModalVisible = ref(false)
const chartRef = ref<HTMLElement | null>(null)
let chartInstance: Line | null = null
const chartLoading = ref(false)
const currentArticleTitle = ref('')

const openDataModal = async (item: any) => {
  currentArticleTitle.value = item.title
  dataModalVisible.value = true
  chartLoading.value = true

  try {
    const res: any = await getRecentArticleDaily(item.id)
    const rawData = res?.data || []
    
    await nextTick()
    await renderChart(rawData)
  } catch (e) {
    message.error('获取文章数据失败')
  } finally {
    chartLoading.value = false
  }
}

const renderChart = async (rawData: any[]) => {
  if (chartInstance) {
    chartInstance.destroy()
    chartInstance = null
  }
  
  if (!chartRef.value || rawData.length === 0) return

  // Sort by date ascending
  const chartData = [...rawData].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
  
  const transformedData: any[] = []
  chartData.forEach(item => {
    transformedData.push({ date: item.date, type: '浏览量', value: item.viewCount || 0 })
    transformedData.push({ date: item.date, type: '点赞数', value: item.likeCount || 0 })
    transformedData.push({ date: item.date, type: '评论数', value: item.commentCount || 0 })
  })

  const { Line } = await loadChartLibrary()

  chartInstance = new Line(chartRef.value, {
    data: transformedData,
    xField: 'date',
    yField: 'value',
    seriesField: 'type',
    smooth: true,
    tooltip: { showMarkers: true },
    point: { size: 3, shape: 'circle' },
  })
  
  chartInstance.render()
}

const handleDataModalClose = () => {
  dataModalVisible.value = false
  if (chartInstance) {
    chartInstance.destroy()
    chartInstance = null
  }
}
</script>

<template>
  <page-container>
    <a-card mb-4>
    </a-card>

    <a-card title="文章管理">
      <template #extra>
        <div style="display: flex; gap: 16px; align-items: center;">
          <a-select
            v-model:value="selectedCategoryId"
            placeholder="按分类筛选"
            style="width: 150px"
            allow-clear
            @change="handleCategoryChange"
          >
            <a-select-option v-for="cat in categories" :key="cat.id" :value="cat.id">
              {{ cat.name }}
            </a-select-option>
          </a-select>
          <a-button type="primary" danger @click="router.push({ name: 'recyclebin' })">
            <DeleteOutlined />
            回收站
          </a-button>
        </div>
      </template>
    </a-card>

    <a-card :bordered="false" class="mt-4 search-result-card" :body-style="{ padding: '24px' }">
      <a-list :loading="loading" :data-source="articleList" item-layout="vertical" size="large" :pagination="pagination">
        <template #renderItem="{ item }">
          <a-list-item :key="item.id" class="article-list-item">

            <template #actions>
              <span key="edit" class="action-btn hover:text-blue-500" @click="handleEdit(item)">
                <EditOutlined />
                <span class="ml-1">编辑</span>
              </span>

              <span key="data" class="action-btn hover:text-green-500" @click="openDataModal(item)">
                <span class="ml-1">数据分析</span>
              </span>

              <span key="comment-manage" class="action-btn hover:text-purple-500" @click="handleCommentManage(item)">
                <span class="ml-1">评论管理</span>
              </span>

              <span key="comment" class="action-btn hover:text-orange-500" @click="toggleCommentStatus(item)">
                <StopOutlined v-if="item.isComment === 1" />
                <CheckCircleOutlined v-else />
                <span class="ml-1">{{ item.isComment === 1 ? '禁止评论' : '允许评论' }}</span>
              </span>

              <a-popconfirm title="确定删除该文章吗？" ok-text="确定" cancel-text="取消" @confirm="handleDelete(item.id)">
                <span key="delete" class="action-btn hover:text-red-500">
                  <DeleteOutlined />
                  <span class="ml-1">删除</span>
                </span>
              </a-popconfirm>
            </template>
            <template #extra>
              <div v-if="item.thumbnail" class="article-cover">
                <img :src="item.thumbnail" alt="cover" loading="lazy" decoding="async" />
              </div>
            </template>

            <div class="article-content">
              <h3 @click="gotoDetail(item.id)"
                class="article-title text-lg font-medium mb-2 cursor-pointer hover:text-blue-600 transition-colors">
                {{ item.title }}
              </h3>

              <div class="mb-3">
                <a-tag color="blue" class="mr-1">{{ item.categoryName }}</a-tag>
                <a-tag v-if="item.isComment == 0" color="orange">禁评</a-tag>
                <a-tag v-else color="blue">允许评论</a-tag>
              </div>

              <div class="text-gray-500 mb-4 line-clamp-3 leading-relaxed">
                {{ item.summary || '暂无摘要' }}
              </div>

              <div class="flex items-center justify-between text-sm text-gray-400">
                <span>创建于 {{ formatTimer(item.createTime) }}</span>
                <span>更新于 {{ formatTimer(item.updateTime) }}</span>
              </div>
            </div>

          </a-list-item>
        </template>
      </a-list>
    </a-card>
    <CrudTableModal ref="crudTableModal" />

    <!-- 数据可视化弹窗 -->
    <a-modal
      v-model:open="dataModalVisible"
      :title="`文章数据 - ${currentArticleTitle}`"
      width="800px"
      :footer="null"
      @cancel="handleDataModalClose"
    >
      <a-spin :spinning="chartLoading">
        <div ref="chartRef" style="height: 400px; margin-top: 16px;"></div>
      </a-spin>
    </a-modal>
  </page-container>
</template>


<style lang="less" scoped>
.system-crud-wrapper {
  .ant-form-item {
    margin: 0;
  }
}

.article-list-item {
  padding: 20px 0;
  transition: all 0.3s;
  border-bottom: 1px solid #f0f0f0;

  &:last-child {
    border-bottom: none;
  }
}

.action-btn {
  cursor: pointer;
  display: inline-flex;
  align-items: center;
  gap: 4px;
  transition: color 0.3s;
  margin-right: 16px; // 增加间距
}

.article-cover {
  width: 200px;
  height: 134px;
  overflow: hidden;
  border-radius: 6px;
  flex-shrink: 0;
  margin-left: 24px;

  img {
    width: 100%;
    height: 100%;
    object-fit: cover;
    transition: transform 0.5s ease;
  }

  .article-list-item:hover & img {
    transform: scale(1.05);
  }
}

@media (max-width: 768px) {
  :deep(.ant-list-item-main),
  :deep(.ant-list-item-extra) {
    display: block;
    margin: 0;
    width: 100%;
  }

  .article-cover {
    width: 100%;
    height: 180px;
    margin-left: 0;
    margin-bottom: 16px;
  }
}
</style>
