    ...
    <div class="relative w-full h-full">
        ...
        <div v-if="activeItem" :class="[
            ...
            <div class="info-header">
                ...
                <div v-if="activeItem.description" class="info-divider"></div>
            </div>

            <!-- 描述区域 -->
            <p v-if="activeItem.description" class="info-description">
                {{ activeItem.description }}
            </p>

            <!-- 操作按钮 -->
            <div class="info-actions">
                <button @click="handleButtonClick" class="action-btn action-btn--primary" :disabled="!activeItem.link">
                ...
                <button v-if="activeItem.link" @click="handleCopyLink" class="action-btn action-btn--secondary"
                    title="复制链接">
                    <svg class="btn-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <!-- 指示器 -->
            <div class="scroll-indicator" v-if="!isMoving">
                <div class="scroll-dots">
                    <span v-for="(_, idx) in resolvedItems" :key="idx"
                        :class="['dot', { 'dot--active': currentItemIndex === idx }]"></span>
                </div>
                <span class="scroll-hint">{{ resolvedItems.length }} 个作品</span>
            </div>
        </div>

        <!-- 拖拽提示 -->
        <div v-if="!activeItem || isMoving" class="drag-hint">
...
class Geometry {
    ...
    addVertex(...args: number[]): this {
        for (let i = 0; i < args.length; i += 3) {
            this.vertices.push(new Vertex(args[i] as number, args[i + 1] as number, args[i + 2] as number));
        }
        return this;
    }
    ...
    addFace(...args: number[]): this {
        for (let i = 0; i < args.length; i += 3) {
            this.faces.push(new Face(args[i] as number, args[i + 1] as number, args[i + 2] as number));
        }
        return this;
    }

    get lastVertex(): Vertex {
        return this.vertices[this.vertices.length - 1] as Vertex;
    }
    ...
    getMidPoint(ndxA: number, ndxB: number, cache: Record<string, number>): number {
        ...
        const a = this.vertices[ndxA]?.position as any;
        const b = this.vertices[ndxB]?.position as any;
...
function createProgram(
    ...
    [gl.VERTEX_SHADER, gl.FRAGMENT_SHADER].forEach((type, ndx) => {
        const shader = createShader(gl, type, shaderSources[ndx] as string);
    ...
    if (attribLocations) {
        for (const attrib in attribLocations) {
            gl.bindAttribLocation(program, attribLocations[attrib] as number, attrib);
        }
    }
...
class InfiniteGridMenu {
    ...
    private init() {
        ...
        this.discVAO = makeVertexArray(
            ...
            [
                [
                    makeBuffer(this.gl, this.discBuffers.vertices as any, this.gl.STATIC_DRAW)!,
                    this.discLocations.aModelPosition as number,
                    3
                ],
                [makeBuffer(this.gl, this.discBuffers.uvs as any, this.gl.STATIC_DRAW)!, this.discLocations.aModelUvs as number, 2]
            ],
    ...
    private animate() {
        ...
        positions.forEach((p, ndx) => {
            ...
            mat4.copy(this.discInstances.matrices[ndx] as any, matrix);
        });
    ...
    private render() {
        ...
        this.gl.uniformMatrix4fv(this.discLocations.uWorldMatrix as any, false, this.worldMatrix);
        this.gl.uniformMatrix4fv(this.discLocations.uViewMatrix as any, false, this.camera.matrices.view);
        this.gl.uniformMatrix4fv(this.discLocations.uProjectionMatrix as any, false, this.camera.matrices.projection);
        this.gl.uniform3f(
            this.discLocations.uCameraPosition as any,
            this.camera.position[0],
            this.camera.position[1],
            this.camera.position[2]
        );
        this.gl.uniform4f(
            this.discLocations.uRotationAxisVelocity as any,
        ...
        this.gl.drawElementsInstanced(
            this.gl.TRIANGLES,
            this.discBuffers.indices?.length as number,
    ...
    private findNearestVertexIndex(): number {
        ...
        for (let i = 0; i < this.instancePositions.length; ++i) {
            const d = vec3.dot(nt, this.instancePositions[i] as any);
            if (d > maxD) {
                maxD = d;
    ...
    private getVertexWorldPosition(index: number): vec3 {
        ...
        return vec3.transformQuat(vec3.create(), nearestVertexPos as any, this.control.orientation);
    }
...
const handleActiveItem = (index: number) => {
    ...
    activeItem.value = items[itemIndex] as any;
};
...
const handleCopyLink = async () => {
    if (!activeItem.value?.link) return;
    try {
        await navigator.clipboard.writeText(activeItem.value.link);
...
.info-title {
    ...
    text-shadow: v-bind('themStore.them.headerTextShadow');
...
.info-divider {
    ...
    border-radius: 2px;
    box-shadow: 0 0 12px v-bind('themStore.them.accentColor');
}
...
.action-btn:hover {
    ...
    box-shadow: v-bind('themStore.them.cardShadowHover');
}
...
.action-btn--secondary:hover {
    background: v-bind('themStore.them.cardBgHover');
    border-color: v-bind('themStore.them.cardBorderHover');
    color: v-bind('themStore.them.textColor');
}
...
.scroll-indicator {
    ...
    margin-top: 8px;
    padding-top: 16px;
    border-top: 1px solid v-bind('themStore.them.cardBorder');
}
...
.dot {
    ...
    height: 6px;
    border-radius: 50%;
    background: v-bind('themStore.them.textTertiary');
...
.dot--active {
    ...
    border-radius: 3px;
    background: v-bind('themStore.them.accentColor');
    box-shadow: 0 0 8px v-bind('themStore.them.accentColor');
}
...
.drag-hint {
    ...
    padding: 20px 32px;
    background: v-bind('themStore.them.contentBgTrans');
    backdrop-filter: blur(10px);
    border: 1px solid v-bind('themStore.them.cardBorder');
    border-radius: 16px;
